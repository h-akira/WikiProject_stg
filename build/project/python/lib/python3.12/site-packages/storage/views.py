...
def get(request, filename):
  ...
  import boto3
  # 権限を確認
  table = boto3.resource('dynamodb').Table(storage_table_name)
  response = table.get_item(
    Key={
      "filename": filename,
    }
  )
  owner = response["Item"]["owner"]
  if "Item" not in response:
    return json_render(request, {"message": "file is not exists"}, code=500)
  if not response["Item"]["public"]:
    if request.auth:
  ...
  try:
    response = s3_client.get_object(
      Bucket=bucket_name,
      Key=os.path.join(settings.AWS["S3"]["key"],f"storage/{owner}/{filename}"), 
    )
  except:
    return json_render(request, {"message": "file is not exists"}, code=500)
...
def post(request, username):
  ...
  Item = {
    ...
    "created": datetime.datetime.now(ZoneInfo("Asia/Tokyo")).strftime("%Y-%m-%d %H:%M:%S")
  }
  table = boto3.resource('dynamodb').Table(storage_table_name)
  response = table.put_item(Item=Item)
  logger.info(f"successed to put item to DynamoDB")
  return json_render(request, {"url": os.path.join("/storage/get", new_file_name)})
