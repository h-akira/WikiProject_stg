...
class Tree:
  ...
  def gen_html(self, username, prettify=True, top=True, a_class=None):
    ...
    text += "<li>"
    if self.end:
      url = reverse_for_slug('wiki:detail', username, self.slug)
      if a_class == None:
...
def gen_tree_htmls(request, table_name, a_white=True):
  htmls = []
  if request.auth:
  ...
  for username in usernames:
    ...
    if response['Count'] == 0:
      continue
    pages = sorted(response['Items'], key=lambda x: x['priority'], reverse=True)
    data = []
    for page in pages:
      data.append(page["slug"].split("/"))
    tree = Tree(username, data=data)
    if a_white:
      htmls.append(tree.gen_html(username, a_class="a-white"))
    else:
      htmls.append(tree.gen_html(username))
  return htmls
